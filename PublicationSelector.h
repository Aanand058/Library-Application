...
public:
  PublicationSelector(const char *title = "Select a publication: ",
                      int pageSize = 15);
  ~PublicationSelector();
  ...
  operator bool() const;
  // clears all the publications and sets the selector empty and ready to be
  // refilled
  void reset();
  // sorts the selections based on date and title
  void sort();
  // runs the selection return the library reference fo the selection
  // publication.
  int run();
};
} // namespace sdds
