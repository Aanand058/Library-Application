...
void PublicationSelector::display(int page) const {
  cout << m_title << endl
       << " LibRef  |LocID | Title                          |Mem ID | Date       "
          "| Author          |"
       << endl
       << "------+------+--------------------------------+-------+------------+"
          "-----------------|"
       << endl;
  for (int i = (page - 1) * m_pageSize; i < m_noOfPubs && i < page * m_pageSize;
       i++) {
    cout.width(4);
    cout.setf(ios::right);
    cout.fill(' ');
...
PublicationSelector::~PublicationSelector() { delete[] m_pub; }
PublicationSelector::PublicationSelector(const char *title, int pageSize)
    : m_pageSize{pageSize} {
...
PublicationSelector &PublicationSelector::operator<<(const Publication *pub) {
  if (m_noOfPubs == m_arraySize) {
    const Publication **temp =
        new const Publication *[m_arraySize += allocationUnit];
...
int PublicationSelector::run() {
  ...
  do {
    ...
    if (retVal && retVal < 3) {
      ...
      // else if (m_currentPage * m_pageSize > m_noOfPubs) //((m_currentPage-1)
      // * m_pageSize > m_noOfPubs)
      //    m_currentPage = m_noOfPubs/m_pageSize +1; // don't go after last
      //    page
    }
...
PublicationSelector::operator bool() const { return m_noOfPubs > 0; }
} // namespace sdds